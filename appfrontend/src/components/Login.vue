  ...
  <div class="min-h-screen bg-gradient-to-br from-brand-50 to-accent-50 pt-32">
    ...
    <div class="max-w-md mx-auto px-6 py-12 bg-white rounded-2xl shadow-xl">
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        <!-- Password Field -->
        <div>
          <label for="password" class="block text-sm font-medium text-brand-700 mb-2">
            Senha
          </label>
          <input
            id="password"
        ...
        <p class="text-center text-brand-600 mt-4">
          Não tem uma conta?
          <router-link :to="{ path: '/register', query: $route.query }" class="text-accent-500 hover:text-accent-600 font-semibold">
            Cadastre-se
          </router-link>
        </p>
      </form>
      ...
      <div class="mt-8">
        <div class="relative">
          ...
          <div class="relative flex justify-center text-sm">
            <span class="px-2 bg-white text-brand-600">
              Ou continue com
            </span>
          </div>
        </div>
...
import { setAuth } from '../auth'; // auth.js functions
...
onMounted(() => {
  // Initialize Google Sign-In
    if (window.google && window.google.accounts && window.google.accounts.id) {
        ...
        window.google.accounts.id.renderButton(
            ...
            { theme: "outline", size: "large", text: "signin_with" } //Adicione o text
        );
    } else {
 ...
 async function handleCredentialResponse(response) {
    try{
        ...
        error.value = error.message || 'Erro ao autenticar com o Google.'; // Set error message
    }
}

async function verifyToken(token) {
  const apiUrl = config.apiBaseUrl + config.api.auth.google; // Simplificado
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return; // Importante: Retorne para evitar o código abaixo
    }

    const data = await response.json();
    // Assuming the backend returns { user: { ... }, token: '...' }
    setAuth(data.token, data.user); // Usa a função setAuth para setar user e token.
...
function handleLogin() {
    // TODO: Implement your email/password login logic here (if you have it)
